...
from core.dev_token.dev_token_update import core_update_api_credit_on_user
...
async def add_credit(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Signature invalide Credit can not be added: {e}"
        )
    ...
    result = core_update_api_credit_on_user(user_id, nb_rows_credited, operation="sum")
    if result is False:
...
async def dev_token(
    ...
    limit = -1
    response: bool = core_insert_api_token(user_id, limit, token)

    if response is False:
        raise HTTPException(status_code=409, detail="A developer token already exists")
    else:
