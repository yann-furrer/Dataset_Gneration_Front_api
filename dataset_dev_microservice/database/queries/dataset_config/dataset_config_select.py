...
SELECT "rulesId", "rulesName" FROM public."RulesConfig" WHERE "datasetConfigId" = :datasetConfigId AND "userId" = :userId;
"""
def select_all_rules_data_by_dataset_config_id(datasetConfigId: str, userId: str) -> bool:
    """
    Récupère le dataset config d'un user à partir du userId et du datasetConfigId
    args : datasetConfigId, userId
    return : yamlContent
    """
    try:
        request = session.execute(text(SELECT_ALL_RULES_DATA_BY_DATASET_CONFIG_ID), {"datasetConfigId": datasetConfigId, "userId": userId})
        row = request.mappings().all()
        return row
    except Exception as error:
         session.rollback()
         print("error -->",error)
