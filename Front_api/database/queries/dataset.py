...
SELECT "datasetNameSystem" FROM public."Dataset" WHERE "datasetConfigId" = :datasetConfigId AND "ownerId" = :ownerId ORDER BY "createdAt" ASC LIMIT 1;
"""
def select_dataset_name_system_with_dataset_id(datasetId: str, userId: str) -> dict:
    try:
        request = session.execute(text(SELECT_DATASET_NAME_SYSTEM_WITH_DATASET_ID), {"datasetConfigId": datasetId, "ownerId": userId})
        result = request.fetchall()
        print(result)
        return result[0]
    except Exception as e:
        session.rollback()
        print(f"Error while selecting dataset name with dataset id {datasetId}", e)
        return False
